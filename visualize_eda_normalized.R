# Program: visualize_eda_normalized.R
...
# Load data ---------------------------------------------------------------
binetflow_bf   <- list.files("data/", "\\d_normalized\\.rds$", full.names = TRUE)
binetflow_base <- binetflow_bf %>% 
  map(read_rds) %>% 
  set_names(gsub("data/(.+)\\.rds", "\\1", binetflow_bf))

binetflow_rf     <- list.files("data/", "_rolled_normalized\\.rds$", full.names = TRUE)
